...
fn dtools(skills_directory: &Path) -> Command {
...
fn sort_subcommand_sorts_trims_and_deduplicates() {
    ...
    let status = dtools(&directory.path().join("skills"))
...
fn uniq_subcommand_deduplicates_in_first_seen_order() {
    let directory = tempfile::tempdir().unwrap();
    let mut child = dtools(&directory.path().join("skills"))
        .arg("uniq")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .spawn()
...
fn installs_a_dynamic_bash_completion_registration() {
    ...
    let output = dtools(&directory.path().join("skills"))
    ...
    assert!(output.status.success());
    let destination = data_home.join("bash-completion/completions/dtools");
    let script = fs::read_to_string(&destination).unwrap();
    assert!(script.contains("COMPLETE=\"bash\""));
    assert!(script.contains("builtin type -P dtools"));
    assert!(
...
fn prints_dynamic_completion_registration_from_the_environment_protocol() {
    ...
    assert!(registration.contains("dtools"));
}
